...
- ~~Rate-limit Credentials sign-in~~ — closes [#64](https://github.com/igortsives/togetherly/issues/64); [`lib/auth/rate-limit.ts`](../lib/auth/rate-limit.ts) tracks failed attempts per `email:<addr>` and `ip:<addr>` in a `SignInAttempt` table; `authorize` returns `null` on limit-exceeded (5 per email, 20 per IP per 15 min). Per-key pruning runs inline; a periodic global cleanup of orphan IP rows is still TODO.
