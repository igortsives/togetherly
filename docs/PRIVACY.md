...
- "Disconnect" MUST: revoke the token with the provider, delete the linked `Account` row, leave imported `CalendarEvent` rows in place (parent can delete those separately), and set the related `CalendarSource.refreshStatus` to a terminal state. The in-product UX lives on the dashboard's Google and Outlook panels (`<details className="dangerZone">` expander). The orchestrator is `disconnectProviderForFamily` in [`lib/sources/disconnect.ts`](../lib/sources/disconnect.ts). Provider revocation is best-effort — Google calls `oauth2.googleapis.com/revoke`, Microsoft calls Graph `/me/revokeSignInSessions`. If revocation fails or is unauthorized, we still delete the `Account` row locally so the user is unblocked.
