...
- ✅ **Round 17 follow-up — Calendar lifecycle management.** Remove an entire Calendar (cascade); trim events before/after a cutoff date; per-source ingest-window floor. Surfaced during interim UAT 2026-05-17 (closes [#148](https://github.com/igortsives/togetherly/issues/148), [#149](https://github.com/igortsives/togetherly/issues/149), [#150](https://github.com/igortsives/togetherly/issues/150)).
- ✅ **Round 18 — Natural-language search.** Free-text input on `/windows` that parses to structured search params via Claude. Shipped 2026-05-17 (closes [#133](https://github.com/igortsives/togetherly/issues/133)).
- ✅ **Round 18 follow-up — LLM cost + connection-pool hardening.** Moved the LLM call outside the source-refresh transaction so a pooled Postgres connection is no longer held idle across the 8-15s round-trip — a brief claim-timestamp lock preserves per-source serialization (closes [#170](https://github.com/igortsives/togetherly/issues/170)). Content-hash short-circuit skips the LLM call when a fetched body is unchanged, cutting realistic LLM spend to ~$5-15/month (closes [#158](https://github.com/igortsives/togetherly/issues/158)).
