...
- [ ] **Cost budget for LLM spend documented**: every HTML/PDF refresh fires the LLM extractor (heuristic fallback was removed 2026-05-17 — see [`DECISIONS.md`](./DECISIONS.md#2026-05-17--remove-heuristic-htmlpdf-extractors-llm-is-the-only-path)). Per-call cost: **~$0.03-0.05** with Claude Sonnet on a typical academic-calendar page. Natural-language search: one Claude call per button press at ~$0.002. Worst-case ceiling with daily cron and the cohort sized at 25-50 families and ~5 sources per family: **~$150/month** if every refresh fires the LLM. The content-hash short-circuit (follow-up) will skip the call when the fetched body is unchanged, bringing realistic monthly spend down toward $5-15/month. Provision a **$200/month Anthropic budget alert** as a safety net until the short-circuit ships, then reduce.
