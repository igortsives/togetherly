...
- [ ] **`ANTHROPIC_API_KEY` provisioned** for the Anthropic project; Claude Sonnet model access verified. **Required** for HTML and PDF source ingestion (Round 17, decision logged 2026-05-17 in [`DECISIONS.md`](./DECISIONS.md#2026-05-17--remove-heuristic-htmlpdf-extractors-llm-is-the-only-path)). Required for natural-language search on `/windows` (MAT-008). The product CANNOT ingest HTML / PDF sources without it — refresh will fail with `HtmlExtractionUnavailableError` / `PdfExtractionUnavailableError`. ICS, Google Calendar, and Outlook Calendar ingestion are unaffected.
- [ ] **Cost budget for LLM spend documented**: HTML/PDF refresh fires the LLM extractor (heuristic fallback was removed 2026-05-17 — see [`DECISIONS.md`](./DECISIONS.md#2026-05-17--remove-heuristic-htmlpdf-extractors-llm-is-the-only-path)). Per-call cost: **~$0.03-0.05** with Claude Sonnet on a typical academic-calendar page. Natural-language search: one Claude call per button press at ~$0.002. The **content-hash short-circuit shipped (issue [#158](https://github.com/igortsives/togetherly/issues/158))**: a scheduled refresh whose fetched body is byte-identical to the last extraction skips the LLM call entirely (a content-addressed PDF upload is never re-extracted). Since most calendars don't change day to day, realistic monthly spend is **~$5-15/month**; the **~$150/month** figure is now only a worst case if every source changed every day. Provision a **$50/month Anthropic budget alert** as a safety net (a sustained breach indicates either an unexpectedly churny source or a volatile-content page defeating the hash — see #158 risks).
